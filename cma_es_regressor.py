...
# --------- 逆向目标：给定 2 维目标指标 ---------
y_target = np.array([330, 5], dtype=np.float64)
...
w = np.array([1.0, 1], dtype=np.float64)  # 两个指标的权重
...
# --------- CMA-ES 设置 ---------
x0 = np.log([15, 200,  200,12, 17, 4])      # 初始均值（可用经验值）
sigma0 = 0.4         # 初始步长（搜索尺度，和变量范围同量纲）
