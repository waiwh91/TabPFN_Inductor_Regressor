...
R_r = TabPFNRegressor(device=torch.device('cuda'))
L_r = TabPFNRegressor(device=torch.device('cuda'))
...
# --------- 逆向目标：给定 2 维目标指标 ---------
y_target = np.array([300, 4], dtype=np.float64)



# --------- 代价函数：让 forward(x) 接近 y_target ---------
# 可加权、可加惩罚、可加正则（例如偏好更小的参数）
w = np.array([1,1], dtype=np.float64)  # 两个指标的权重
...
def objective(x: np.ndarray) -> float:
    ...
    # diff = np.abs(y[0] -y[2]) * np.abs(y[0] -y[2])
