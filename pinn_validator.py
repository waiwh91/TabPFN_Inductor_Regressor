...
pinn_inter = PINN()
pinn_inter.load_state_dict(torch.load('models/saved/PINN_inter_model.pth'))
...
inter_R, inter_L = np.exp(pinn_inter(torch.log(data_np[:,:6]),torch.log(data_np[:,6])).T.detach().numpy())



rr_p = rr_reg.predict(np.log(data.iloc[:,:7].to_numpy()))
para_p = para_reg.predict(np.log(data.iloc[:,:7].to_numpy()))
para_lp = para_l.predict(np.log(data.iloc[:,:7].to_numpy()))
